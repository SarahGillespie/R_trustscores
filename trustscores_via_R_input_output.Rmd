...
# SARAH: TypeError: Cannot broadcast np.ndarray with operand of type <class 'list'>

# Error soure: trustscore.py line 80
...
# so...
# the x argument needs to be an array ---> not a df??
# the y argument needs to be "corresponding labels" but the doc string does not provide a specific data structure.

# to do: convert X_penguins from 'pandas.core.frame.DataFrame' to an array. perhaps a numpy array?

# newest error: ValueError: Found array with 0 sample(s) (shape=(0, 1)) while a minimum of 1 is required.
```
