...
Download the entire repository. It is important to have the trustscores.py and trustscore_evaluation.py in the same folder as the .Rmd file to import the trust score functions that are in that Python file.
...
```{python calculate specific trust scores}
# Python code chunk

# GIVEN X VALUE
# pick a specific X value from the given data
X_value = np.array([X_penguins[1]])
...
# Compute trusts score from the above model, given (unlabeled) testing examples and (hard) model predictions.
trust_score_specific = trust_model.get_score(X_value, y_pred_single)
...
X_value_theoretical = np.array([[40, 17, 180.0, 3000.0,]])
